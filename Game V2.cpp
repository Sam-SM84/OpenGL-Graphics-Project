    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!inventory)
        {
            ...
            for (auto obj = objects.begin(); obj != objects.end(); obj++)
            {
                ...
                float b = player.direction.z;
                float a1 = player.direction.x / glm::cos(glm::radians(FOV));
                float a2 = player.direction.x / glm::cos(glm::radians(FOV + 90.0f));
                bool putInList = false;

                // Too near
                if (glm::distance(player.Position, obj->modelPosition) < obj->radius) putInList = true;

                // In triangle
                else if (glm::dot(glm::normalize(player.direction), objectVector) >= limitAngle) putInList = true;
                
                // Near edges
                else if ((a1 * obj->modelPosition.x) + (b * obj->modelPosition.z) < obj->radius) putInList = true;
                else if ((a2 * obj->modelPosition.x) + (b * obj->modelPosition.z) < obj->radius) putInList = true;

                if (putInList) visibleObjects.push_back(*obj);
            }
            */
